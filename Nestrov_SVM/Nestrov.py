...
class Runge_Kutta_SVM():
    ...
    def Runge_Kutta(self, y, c0):
        loss_history = [0]*self.epochs
        V = self.Sub_Gradient(y,c0)
        for epoch in range(2,self.epochs):
            c = c0 - self.learning_rate*V
            V = (epoch -1)/(epoch+2)*V + (3)/(epoch+2)*self.Sub_Gradient(y,c)
            yhat = predictor(self.X_train, c)
    ...
    def SVM_OVR_train(self):# y_train: one_hot_encoder labels
        ...
        np.savetxt('loss_Ns.txt',self.history_loss,fmt='%0.8f')
