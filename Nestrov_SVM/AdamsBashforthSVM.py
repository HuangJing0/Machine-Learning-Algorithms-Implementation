...
class Runge_Kutta_SVM():
    ...
    def Runge_Kutta(self, y, c1, c0):
        ...
        for epoch in range(3,self.epochs):
            V = V1 - 2/3*((epoch-1)/(epoch-2)*V1+ self.learning_rate * self.Sub_Gradient(y, c1)) + 1/2*((epoch-1)/(epoch-2)*V0+ self.learning_rate * self.Sub_Gradient(y, c0))
            c = c1 + self.learning_rate/2*(3*V1 - V0)
            yhat = predictor(self.X_train, c)
    ...
    def SVM_OVR_train(self):# y_train: one_hot_encoder labels
        ...
        np.savetxt('loss_Adams.txt',self.history_loss,fmt='%0.8f')
