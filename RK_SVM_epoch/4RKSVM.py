...
class Runge_Kutta_SVM():
    ...
    def Runge_Kutta(self, y, c):
        loss_history = 0
        epoch = 1
        k1 = -self.Sub_Gradient(y, c)
        k2 = -self.Sub_Gradient(y, c + self.learning_rate*k1/2)
        k3 = -self.Sub_Gradient(y, c + self.learning_rate*k2/2)
        k4 = -self.Sub_Gradient(y, c + self.learning_rate*k3)
        c = c + self.learning_rate*(k1/6 +k2/3+k3/3+k4/6)
        yhat = predictor(self.X_train, c)
    ...
    def SVM_binary_train(self, y_train_one_column):
        ...
        coeffs_grad, epoch = self.Runge_Kutta(y_train_one_column, coeffs_0)
        print(epoch)
        return coeffs_grad
