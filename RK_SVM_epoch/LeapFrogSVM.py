...
class Runge_Kutta_SVM():
    def __init__(self, X_train, y_train, lamda, epochs, learning_rate, tolerance = 1e-3, n = 100):
    ...
    def Runge_Kutta(self, y, c1, c0):
        ...
        loss_history = self.loss(y, c0, yhat).ravel()
        yhat = predictor(self.X_train, c1)
        loss_history = self.loss(y, c1, yhat).ravel()
        epoch = 2
        gradient = self.Sub_Gradient(y, c1)
        c = c0 - 2*self.learning_rate*(gradient)
        yhat = predictor(self.X_train, c)
        loss_now = self.loss(y, c, yhat).ravel()
        c0 = c1
        c1 = c
        epoch = epoch + 1
        while (abs(loss_now - loss_history) > self.tolerance):
            loss_history = loss_now
            gradient = self.Sub_Gradient(y, c1)
            c = c0 - 2*self.learning_rate*(gradient)
            yhat = predictor(self.X_train, c)
            loss_now = self.loss(y, c, yhat).ravel()
    ...
    def SVM_binary_train(self, y_train_one_column):
        ...
        coeffs_grad, epoch = self.Runge_Kutta(y_train_one_column, coeffs_0,coeffs_1)
        print(epoch)
        return coeffs_grad
...
mySVM = Runge_Kutta_SVM(X_train_norm, y_train_ohe, lamda=0.01, epochs=10000, learning_rate=0.01)
mySVM.SVM_OVR_train()
