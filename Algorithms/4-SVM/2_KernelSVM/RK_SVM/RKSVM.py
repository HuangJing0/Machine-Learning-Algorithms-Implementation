...
class Runge_Kutta_SVM():
    ...
    def Runge_Kutta(self, y, c1, c0):
        ...
        c[1] = c0
        c[2] = c1
        yhat = predictor(self.X_train, c[1])
        loss_history[1] = self.loss(y, c[1], yhat).ravel()
        yhat = predictor(self.X_train, c[2])
        loss_history[2] = self.loss(y, c[2], yhat).ravel()
        for epoch in range(3,self.epochs):
            gradient = Sub_Gradient(self, y, c[epoch-1])
            c[epoch] = 2*self.learning_rate(c[epoch-2] + gradient)
            yhat = predictor(self.X_train, c[epoch])
            loss_history[epoch] = self.loss(y, c[epoch], yhat).ravel()
...
mySVM = Runge_Kutta_SVM(X_train_norm, y_train_ohe, lamda=0.01, epochs=10, learning_rate=0.01)
mySVM.SVM_OVR_train()
