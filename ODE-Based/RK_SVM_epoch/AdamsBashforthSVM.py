...
class Runge_Kutta_SVM():
    ...
    def Runge_Kutta(self, y, c1, c0):
        ...
        gradient1 = self.Sub_Gradient(y, c1)
        gradient0 = self.Sub_Gradient(y, c0)
        c = c1 - self.learning_rate/2*(3*gradient1 - gradient0)
        yhat = predictor(self.X_train, c)
        loss_now = self.loss(y, c, yhat).ravel()
        c0 = c1
        c1 = c
        epoch = 1
        while (abs(loss_history-loss_now) > self.tolerance):
